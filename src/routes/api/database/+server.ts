...
export const POST: RequestHandler = async ({ request }) => {
  ...
  if (action === 'download') {
    ...
    const rec = (registry as any)?.[dbId];
    if (!rec || rec._type !== 'database') return Response.json({ error: 'Not found' }, { status: 404 });
    const { data } = await downloadFileFromTelegram(rec.telegramFileId);
    return new Response(data, {
      headers: {
        'Content-Type': 'application/x-sqlite3',
        'Content-Disposition': `attachment; filename="${rec.name}.sqlite"`,
      }
    });
  }
